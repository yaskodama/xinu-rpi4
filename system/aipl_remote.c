...
/* ---- 応答の控え（at-most-once） ------------------------------------------
   ...
   控えを返す。これで「高々一度」になる。Pi 5 には元からあり、Pi 4 に無かった。
   gather(...) は同報して再送するので、これが無いと数え上げが狂う。 */
#define ANS_MAX 4
                     ...
                     const char *arg, long timeout_ms, char *out, int outcap)
{
    ...
    { unsigned long last_tx = t0;
      while (!g_have) {
          if ((long)(mesh_now_ms() - t0) >= timeout_ms) { g_wait_id = -1; g_n_timeout++; return -2; }
          net_rx_pump();
...
static int mesh_run(const char *line, int len, int id, char kind, long ms)
{
    ...
    /* Pi 5 の net_rx_remote_window（割り込みにも汲ませる窓）は Pi 4 には無い。
       Pi 4 はアクターが実プロセスなので、受信は net_rx_pump() だけで足りる。 */
