/* system/browser.c —— 機内ブラウザ（Pi 4 版。xinu-rpi5 の system/browser.c を移植）
 * Pi 5 との違い:
 *   ・受信は IRQ で起きる net プロセスが行う（先取りは OFF なので、待つ側は
 *     proc_yield() で譲る。環を自分で汲まない）。genet_rx_tick の連鎖の先頭に browser_handle。
 *   ・IP/経路/DNS は DHCP から（Pi 5 は静的 IP）。GENET の統計は無い（計器は 0）。
 *   ・板名は "Pi4"。
 *
...
extern void proc_yield(void);        /* net プロセスに譲る（Pi 4 は先取り OFF） */
...
static void br_wait_us(unsigned long us)
{
    ...
    /* Pi 4: 受信は net プロセスが行う（RX 割り込みで起こされ、こちらが譲ると走る）。
       自分で環を汲むと net プロセスと二重に汲んで環を壊すので、譲るだけ。 */
    do { proc_yield(); if (br_via_wifi) wifi_net_poll(); } while ((long)(br_now_us() - t0) < (long)us);
}
...
/* ===== 経路の設定 ======================================================
 * Pi 4 は DHCP で IP/マスク/ルータ/DNS を受け取る（192.168.3.100）。
...
int browser_netinfo(char *d, int cap)
{
    ...
    /* Pi 4 (GENET) には RP1 GEM の統計レジスタが無い。0 のまま */
...
/* HTTP（/browse?url=）からの要求。app プロセスの中で取りに行くと net プロセスと
 * 待ち合わせが絡むので、クリックと同じく保留にして wm の巡回で取る。 */
