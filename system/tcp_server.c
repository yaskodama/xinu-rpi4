...
struct tcp_conn {
    ...
    int greeted;
    int via;                    /* 0=有線 1=無線（メッシュから来た接続。返事も無線へ） */
                    ...
                    unsigned char flags, const char *payload, int payload_len)
{
    ...
    if (c->via) { extern void wifi_macaddr(unsigned char *); extern void wifi_ipaddr(unsigned char *);
    ...
    unsigned short ucs = tcp_checksum(src_ip, c->peer_ip,
    ...
    {
        int rc;
        if (c->via) { extern int wifi_eth_tx(const unsigned char *, int); rc = wifi_eth_tx((const unsigned char *)tx_frame, send_len); }
        else rc = genet_tx_frame((const unsigned char *)tx_frame, send_len);
        irq_restore(daif_tx);
        return rc;
    }
}
...
/* ---- Memory-fill benchmark (cross-board: does bulk store scale on A72?) ----
 * Identical workload to the Pi 3's smpbench fill: a plain buffer written 8
 * times.  With the D-cache off (as on both boards) every store is a RAM
...
#define BENCH_FILL_WORDS  (256 * 1024)   /* 1 MB, matches Pi 3 */
...
/* Pool round-trip: does nothing, so the N-core time is the dispatch+collect
 ...
 * orchestrator can hand each board a disjoint column range and sum the partials.
 * Native (no JIT deadline) → correct for any n; SMP across cores. */
 ...
 * the Pi 3's clkcount() so the two boards' numbers are comparable. */
...
static int http_build(const char *req, char *out, int max)
{
    ...
    if (starts_with(req, "POST /cc") || starts_with(req, "GET /cc")) {
        ...
        { extern const char *kernel_build_id(void);
          ...
          bl = s_put(body, bl, " retx=");          bl = s_putdec(body, bl, (long)st[7]);
          bl = s_put(body, bl, "\n"); }
        ...
        int is_fill   = (kind[0] == 'f');
        int is_null   = (kind[0] == 'x');   /* kind=x... : pool round-trip */
        /* cores caps the parallel run for an Amdahl sweep; 0 = all online.
         * Matches the Pi 3's /bench?cores=N so both boards sweep identically. */
         ...
         *   curl 'http://192.168.3.100/nqpart?n=13&c0=4&c1=8' */
        ctype = "text/plain";
        int online = smp_cores_online();
        ...
        { extern int xhci_msd_present(void); extern int xhci_enum_journal(char *, int);
          bl = s_put(body, bl, " msd_present="); bl = s_putdec(body, bl, (long)xhci_msd_present());
          char jr[768]; int jn = xhci_enum_journal(jr, (int)sizeof jr); (void)jn;
          bl = s_put(body, bl, "\nenum: "); bl = s_put(body, bl, jr); }
        bl = s_put(body, bl, "\n");
...
int tcp_handle_packet_via(const unsigned char *frame, int len, int via)
{
    ...
    if (ip[16] != g_my_ip[0] || ip[17] != g_my_ip[1] ||
        ...
        int mine = 0;                                 /* 無線の IP 宛（メッシュ上の板からの接続）も受ける */
