// include/smp.h — worker-pool SMP for the BCM2711's 4 Cortex-A72 cores.
//
// Architecture (see docs/SMP_REPORT_JA.md):
...
//     every access goes straight to RAM, so plain `volatile` + `dmb`/`dsb`
//     barriers are coherent across cores — no cache protocol, no spinlocks.
 ...
 * Safe to call even if the secondary cores never respond — they just stay
 * offline and parallel jobs fall back to running on core 0. */
void smp_init(void);
