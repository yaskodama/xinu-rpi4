...
void *getmem(unsigned long nbytes)
{
    ...
    while (curr != 0) {
         ...
         * end of curr and clobber the successor.  Sizes are 8-byte granular
         * (ROUNDMB) while the header is 16, so an 8-byte remainder is
         * reachable — take the whole block instead of splitting. */
