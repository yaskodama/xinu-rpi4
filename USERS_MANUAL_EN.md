...
**Difference from the Pi 5.** On the Pi 4 an actor is a **real Xinu process**,
so waiting for the actor that was given a `replyto` runs `ap_run()` (yield until
every actor is blocked) rather than the Pi 5's cooperative pump. What the user
sees is the same.
