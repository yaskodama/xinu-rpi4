...
- **`/microsd` と `/sd` が空**。SD の FAT32 列挙が成立していない
  （`NEXT_SESSION_PI4.md` の既知症状と一致）。⑤の実機検証を阻んでおり、FAT32 経路が
  実機で一切テストできない。**次の本命**。
- **メッシュ（MANET）でのアクター実行は未測定**。Pi3・Pi5 が停止しており、Pi4 単体では
  `/manet` が `node=0 peers=0`。複数ノードに跨るアクター分散実行のベンチマークは次回の課題。
