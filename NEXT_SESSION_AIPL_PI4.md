...
次に試すこと:
1. `while (i < 1024)` を `while (i < g_nobj)`（実在数）に変えて再現するか見る
   ―― abcl2c は三機共通なので、直せば Pi 3・Pi 5 にも効く（無害）
2. 素の C を `/compile` に投げて `g_obj[2048]` の端を読み書きし、
   Pi 4 の JIT が本当に 2048 個ぶん確保しているか確かめる（板を落とさず測れる）
3. `select` に `timeout` 節が無い版（＝`__sel_sweep` を出さない）で再現するか
